...
def getVacancyAge(properties,rooms,qdata):
    ...
    for propertyId in propertyIds:
        ...
        if (individualProperty.shape[0] == 0):
            #         print(propertyId)
            fromDate = datetime.datetime.strptime(str(properties.at[propertyId, 'availfrom']), '%Y-%m-%d').date()
        ...
        if (len(roomIds) == 0):
            ...
            for exit_date in exit_dates_str:
                if((exit_date) == None):
                    ...
                    fromDate = datetime.datetime.strptime(str(exit_date), '%Y-%m-%d').date()
                ...
                if (rooms.at[roomId, 'availablebed'] == 0):
                    ...
                    for x in exitDates:
                        ...
                        if(x == None):
                            ...
                            fromDate = datetime.datetime.strptime(str(x), '%Y-%m-%d').date()
...
def getImages(propertyIds, qdata):
    ...
    ################################### SQL Method #######################################
    images = pd.read_sql_query(query, engine, index_col='id')
...
def getRoomDetails(properties,rooms,propertyIds,family):
    ...
    room = pd.DataFrame([], columns=['rooms','minimumRent','maximumRent'], index=propertyIds)
    ...
    for i in propertyIds:
        ...
        if(family == 0 and (len(list(SpecificRoom.rent)) != 0)):
            room.at[i, 'minimumRent'] = min(list(SpecificRoom.rent))
            room.at[i, 'maximumRent'] = max(list(SpecificRoom.rent))
        else:
            room.at[i, 'minimumRent'] = properties.at[i, 'rent']
...
def getRecommendedProperties(request):
    ...
    try:
        ...
        if (propertyType == '4'):
            ...
            query = "select * from properties p where p.city = \'" + city + "\' and p.status = 'available'"
        elif (gender != '0'):
        ...
        commClassify = pd.DataFrame(np.zeros((properties.shape[0], 3), dtype=int),
                                    columns=['commisionLessThan14', 'commisionEqualTo14', 'commisionGreaterThan14'],
        ...
        # print(propertyIds)
        roomsDF = pd.read_sql_query(rooms_query, engine, index_col='id')
        properties = properties[properties.status.isin(statuses)]
        roomsDF = roomsDF[roomsDF.property_id.isin(propertyIds)]
        ...
        area = pd.DataFrame(data=np.zeros((properties.shape[0], 3), dtype=int), index=properties.index,
                            columns=areaTypes)
        for i in properties.index:
        ...
        golden = pd.DataFrame(data=np.zeros((properties.shape[0]), dtype=int), index=properties.index,
                              columns=['isGolden'])
        ...
        vacancyAgeDF = pd.DataFrame(np.zeros((properties.shape[0], 3), dtype=float),
                                    columns=['vacancyAgeLessThan50', 'vacancyAgeLessThan120',
                                             'vacancyAgeGreaterThan120'],
                                    index=properties.index)
        ...
        if (variant == 1 or variant == 2):
            budgets = (np.zeros(len(budgetList))).tolist() + [wDistanceLessThan2KM, wDistanceLessThan5KM,
                                                              wDistanceLessThan10KM, wManaged, wMarketPlaced,
                                                              wAreaLessThan1000, wAreaLessThan1700,
                                                              wAreaGreaterThan1700, wNonGoldenProperties,
                                                              wVacancyAgeLessThan50, wVacancyAgeLessThan120,
            ...
            budgets = (np.zeros(len(budgetList))).tolist() + [wDistanceLessThan2KM, wDistanceLessThan5KM,
                                                              wDistanceLessThan10KM, wManaged, wMarketPlaced,
                                                              wAreaLessThan1000, wAreaLessThan1700,
                                                              wAreaGreaterThan1700, wGoldenProperties,
                                                              wVacancyAgeLessThan50, wVacancyAgeLessThan120,
                                                              wVacancyAgeGreaterThan120, wCommisionLessThan14,
                                                              wCommisionEqualTo14, wCommisionGreaterThan14]
